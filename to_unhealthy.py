	...
	for dicts in ingredients:
		...
		for subst in to_unhealthydict:
			if subst in dicts['name'].lower():
				...
				dicts['name']=dicts['name'].lower().replace(subst, to_unhealthydict[subst])
	...
	for dcts in directions:
		...
		for ing in to_unhealthydict:
			if ing in dcts['original'].lower():
				dcts['original']=dcts['original'].lower().replace(ing,to_unhealthydict[ing])
				directions[countd]['original']=dcts['original']
			if ing in dcts['method'].lower():
				dcts['method']=dcts['method'].lower().replace(ing,to_unhealthydict[ing])
				directions[countd]['method']=dcts['method'] 
		for ingredient in dcts['ingredients']:
			countpi+=1
			for ing in to_unhealthydict:
				if ing in ingredient.lower():
					ingredient=ingredient.lower().replace(ing, to_unhealthydict[ing])
